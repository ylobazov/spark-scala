package com.github.ylobazov.spark.mlib
...
object LinearRegressionRDD {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setMaster("local[*]").setAppName("LinearRegression").set("spark.driver.host", "localhost")
    ...
    //Evaluate Squared error - the quality measure (the less the better)
    val testPointsNum = testData.count()
    val squaredError = predictionAndLabel.map(x => x._1 - x._2).map(x => x * x).sum() / testPointsNum
    println(s"SQUARED ERROR: $squaredError")
  }
}
